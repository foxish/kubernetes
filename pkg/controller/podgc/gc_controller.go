...
func (gcc *PodGCController) gcUnscheduledTerminating(pods []*api.Pod) {
	...
	for _, pod := range pods {
		...
		glog.V(2).Infof("Found unscheduled Pod %v not assigned to any Node. Deleting.", pod.Name)
		if err := gcc.deletePod(pod.Namespace, pod.Name); err != nil {
			utilruntime.HandleError(err)
		} else {
			glog.V(4).Infof("Forced deletion of unscheduled Pod %s succeeded", pod.Name)
		}
	}
}
